...
const RulesList = ({ rules, onRemove, onAdd }: { rules: Pick<StreamRule, "value" | "tag">[], onRemove?: (value: string) => void, onAdd?: (value: string, tag?: string) => void }) => {
  return (
          ...
          <tbody>
            {rules.map((rule, index) => (
              <tr key={index} className="border-b">
                ...
                {onAdd && <td className="px-4 py-2 text-center"><button onClick={() => onAdd(rule.value, rule.tag)}>
